 ...
 *
 * $Header: /cvsroot/ebxmlrr/omar/src/java/org/freebxml/omar/common/security/xwssec10/SecurityCallbackHandler.java,v 1.4 2005/06/11 00:20:53 geomurr Exp $
 * ====================================================================
 */
...
public class SecurityCallbackHandler implements CallbackHandler{
    ...
    public SecurityCallbackHandler(CredentialInfo credentialInfo) {
        if (credentialInfo == null) {
            throw new IllegalArgumentException(CommonResourceBundle.getInstance().getString("message.nilCredentialInfo"));
        }
    ...
    public void handle(Callback callbacks[]) {
        for (int i = 0; i < callbacks.length; i++) {
            if (log.isDebugEnabled()) {
                log.debug("Handling callback: " + callbacks[i].getClass().getName());
            }
            ...
            // property callback (for timestamp)
                ...
                if (maxClockSkew != null) {
                    try {
                        ...
                        log.warn(CommonResourceBundle.getInstance().getString("message.InvalidValueForProperty", new Object[]{KEY_MAX_CLOCK_SKEW, maxClockSkew}), e);
                    }
                }
            ...
            // unsuported
                ...
                log.warn(CommonResourceBundle.getInstance().getString("message.UNSUPPORTEDCallback", new Object[]{callbacks[i].getClass().getName()}));
            }
        }
    }
    ...
    private class X509CertificateValidatorImpl implements CertificateValidationCallback.CertificateValidator {
        ...
        throws CertificateValidationCallback.CertificateValidationException {
            ...
            try {
                ...
                e.printStackTrace();
                throw new CertificateValidationCallback.CertificateValidationException(CommonResourceBundle.getInstance().getString("message.X509CertificateExpired"), e);
            } catch (CertificateNotYetValidException e) {
                e.printStackTrace();
                throw new CertificateValidationCallback.CertificateValidationException(CommonResourceBundle.getInstance().getString("message.X509CertificateNotValid"), e);
            }
